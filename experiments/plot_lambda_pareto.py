...
ax1.axhline(y=sum(add_baseline)/len(add_baseline), color='green', linestyle=':', alpha=0.4,
...
fig.suptitle('Lambda EWC Sensitivity: Pareto Frontier is Flat for Two Tasks',
             fontsize=15, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(r'C:\Users\Admin\tabula-rasa\experiments\lambda_sweep_pareto.png', dpi=150, bbox_inches='tight')
...
print(f'  {"-"*6}-+-{"-"*9}-+-{"-"*10}-+-{"-"*10}')
for i, lam in enumerate(lambdas):
    print(f'  {lam:>6} | {add_baseline[i]:>8.0f}% | {add_retention[i]:>9.0f}% | {sub_learning[i]:>9.0f}%')
