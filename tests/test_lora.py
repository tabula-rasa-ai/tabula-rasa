...
def test_lora_forward():
    ...
    assert torch.allclose(out_base, out_lora_init, atol=1e-6), (
    ...
    assert not torch.allclose(out_base, out_lora_tweaked, atol=1e-4), (
...
def test_apply_lora_to_model():
    ...
    assert len(lora_layers) == original_linears, (
    ...
    assert lora_count == original_linears, (
...
def test_lora_save_load():
    ...
    try:
        ...
        for i, (l1, l2) in enumerate(zip(lora_layers1, lora_layers2)):
            assert torch.allclose(l1.lora_A, l2.lora_A, atol=1e-6), (
                f'Layer {i}: lora_A mismatch'
            )
            assert torch.allclose(l1.lora_B, l2.lora_B, atol=1e-6), (
                f'Layer {i}: lora_B mismatch'
            )
        ...
        assert torch.allclose(logits1, logits2, atol=1e-5), (
            'Logits mismatch after save/load cycle'
        )
    finally:
...
def test_lora_multiple_adapters():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert torch.allclose(out_loaded_a, out0, atol=1e-5), (
        ...
        assert torch.allclose(out_loaded_b, out1, atol=1e-5), (
            'Switched adapter B should match original B'
        )
        # After switching to B, output should NOT match A
        assert not torch.allclose(out_loaded_b, out0, atol=1e-4), (
...
def test_lora_edge_cases():
    ...
    out_base = base(x)
    assert torch.allclose(out_zero, out_base, atol=1e-6), (
