...
try:
    from egefalos.socratic_critique import SocraticCritiqueLoop, generate_problem
    HAS_CRITIQUE = True
except ImportError:
    HAS_CRITIQUE = False
...
class SocraticSelfTrainer:
        ...
        for i, (expr, expected_str, expected_int) in enumerate(problems):
            ...
            if hint_idx >= 0 and hint_idx < len(telemetry.rounds):
                ...
                if prev_round.critic_errors:
                    from egefalos.socratic_critique import construct_hint
    ...
    def _quick_eval(self, num: int = 50) -> float:
        ...
        from egefalos.socratic_critique import generate_problem as gen_prob
        self.model.eval()
        correct = 0
        for _ in range(num):
