...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["syntax", "fuzzing", "algorithm"],
    ...
    print(
        f"[CodeAZ] Model created: {sum(p.numel() for p in model.parameters()):,} params", flush=True
    )
    ...
    if isinstance(results, dict):
        for stage_key, stage_results in (
            results.items() if "stage" not in results else [(None, results)]
        ):
