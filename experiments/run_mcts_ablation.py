    ...
    while global_step < cfg.max_steps:
        for x, y in train_loader:
            ...
            if global_step % cfg.log_every == 0:
                ...
                print(
                    f"      Step {global_step}/{cfg.max_steps} | loss={avg_loss:.4f} | {elapsed:.0f}s",
                    flush=True,
                )
...
def generate_with_mcts(
    model, tok, prompt, n_simulations=16, max_new_tokens=10, top_k=5, max_rollout_tokens=8
):
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick validation mode: 10 problems only"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    if ckpt_file.exists():
        ...
        model.load_state_dict(
            sd["model_state_dict"] if "model_state_dict" in sd else sd, strict=False
        )
        model.eval()
    ...
    methods = [
        ...
        (
            "Micro-MCTS(16)",
            lambda m, tok, prompt: generate_with_mcts(m, tok, prompt, n_simulations=16),
        ),
    ...
    for method_name, method_fn in methods:
        ...
        print(
            f"acc={acc:.1f}% | time={avg_time:.4f}s/token | {avg_tokens:.1f} tok | wall={wall:.1f}s"
        )
    ...
    for method_name, _ in methods:
        ...
        print(
            f'  {method_name:<20} {r["accuracy_pct"]:>10} {r["avg_time_per_problem_s"]:>8.4f}s {r["avg_tokens"]:>12.1f}'
        )
