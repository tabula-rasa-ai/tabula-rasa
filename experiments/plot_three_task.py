...
ax2.annotate(
    "−6% loss",
    xy=(2, fisher_norms[2]),
    xytext=(1.5, fisher_norms[2] - 0.15),
    fontsize=12,
    color="red",
    fontweight="bold",
...
fig.suptitle(
    "Scalability Boundary: Information Loss During Fisher Merge",
    fontsize=14,
    fontweight="bold",
    y=1.02,
)
plt.tight_layout()
plt.savefig(
    r"C:\Users\Admin\tabula-rasa\experiments\three_task_scalability.png",
    dpi=300,
    bbox_inches="tight",
)
...
print(f'  {"-"*20}-+-{"-"*6}-+-{"-"*6}-+-{"-"*6}-+-{"-"*8}')
print(
    f'  {"After Addition":<20} | {add_baseline:>5.0f}% | {"-":>6} | {"-":>6} | {fisher_norms[0]:>8.2f}'
)
print(
    f'  {"After Subtraction":<20} | {after_sub["add"]["accuracy"]:>5.0f}% | {after_sub["sub"]["accuracy"]:>5.0f}% | {"-":>6} | {fisher_norms[1]:>8.2f}'
)
print(
    f'  {"After Multiplication":<20} | {after_mul["add"]["accuracy"]:>5.0f}% | {after_mul["sub"]["accuracy"]:>5.0f}% | {after_mul["mul"]["accuracy"]:>5.0f}% | {fisher_norms[2]:>8.2f}'
)
print()
