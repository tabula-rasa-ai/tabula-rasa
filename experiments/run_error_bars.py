...
def run_experiment(label, use_ewc, lambda_val):
    ...
    for seed in SEEDS:
        ...
        # Train
        add_final, sub_final, traj, steps = train_subtraction(
            model, cfg, tok, use_ewc, lambda_val, seed
        )
        results.append(
            {
                "seed": seed,
...
def main():
    ...
    for key, r in all_results.items():
        print(
            f'  {r["label"]:<25} | {r["addition_mean"]:>7.1f}%  | ±{r["addition_std"]:.1f}% | {r["subtraction_mean"]:>7.1f}%  | ±{r["subtraction_std"]:.1f}%'
        )
    ...
    for key, r in all_results.items():
        ...
        print(
            f'    Retention drop: {r["retention_drop_mean"]:.1f} ± {r["retention_drop_std"]:.1f} pp'
        )
