...
def main():
    ...
    print(
        f"  After sub: add={step3['measurements']['add']['accuracy']:.1f}%  "
    ...
    print(
        f"  Fisher norm: {fisher_norm:.4f} → {merged_norm:.4f} (growth: {merged_norm/fisher_norm:.2f}x)"
    )
    ...
    print(
        f"  After mul: add={step5['measurements']['add']['accuracy']:.1f}%  "
    ...
    print(f"  {'-'*12}-+-{'-'*6}-+-{'-'*6}-+-{'-'*6}-+-{'-'*8}")
    print(
        f"  {'After Add':<12} | {after_add['add']['accuracy']:>5.1f}% | {'-':>6} | {'-':>6} | {fisher_norm:>8.2f}"
    )
    print(
        f"  {'After Sub':<12} | {after_sub['add']['accuracy']:>5.1f}% | {after_sub['sub']['accuracy']:>5.1f}% | {'-':>6} | {merged_norm:>8.2f}"
    )
    print(
        f"  {'After Mul':<12} | {after_mul['add']['accuracy']:>5.1f}% | {after_mul['sub']['accuracy']:>5.1f}% | {after_mul['mul']['accuracy']:>5.1f}% | {step5['fisher_norm']:>8.2f}"
    )
    ...
    if min_retained > 80:
        ...
        print(
            f"  Fisher grows linearly: {fisher_norm:.1f} → {merged_norm:.1f} → {step5['fisher_norm']:.1f}"
        )
    else:
