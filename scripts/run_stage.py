...
def main():
    ...
    parser.add_argument(
        "--stage", type=int, choices=range(1, 7), help="Developmental stage to train (1-6)"
    )
    ...
    parser.add_argument(
        "--check", action="store_true", help="Run graduation check on current stage"
    )
    parser.add_argument(
        "--spawn",
        action="store_true",
    ...
    if stage == 1:
        ...
        from egefalos.pattern_specialist import evaluate_patterns, train_pattern_specialist

        model = MathTransformer(cfg)
        tok.save(str(Path(info["dir"]) / "tokenizer.json"))
        results = train_pattern_specialist(
            model, tok, train_steps=steps, batch_size=batch, learning_rate=lr
        )
        acc = evaluate_patterns(model, tok, num_problems=200)
        print(f"\n  Final accuracy: {acc*100:.1f}%")

    elif stage == 3:
        from egefalos.taxonomy_specialist import evaluate_taxonomy, train_taxonomy_specialist

        model = MathTransformer(cfg)
        tok.save(str(Path(info["dir"]) / "tokenizer.json"))
        results = train_taxonomy_specialist(
            model, tok, train_steps=steps, batch_size=batch, learning_rate=lr
        )
        ...
        from egefalos.state_specialist import evaluate_state, train_state_specialist

        model = MathTransformer(cfg)
        tok.save(str(Path(info["dir"]) / "tokenizer.json"))
        results = train_state_specialist(
            model, tok, train_steps=steps, batch_size=batch, learning_rate=lr
        )
        ...
        from egefalos.grammar_specialist import evaluate_grammar, train_grammar_specialist

        model = MathTransformer(cfg)
        tok.save(str(Path(info["dir"]) / "tokenizer.json"))
        results = train_grammar_specialist(
            model, tok, train_steps=steps, batch_size=batch, learning_rate=lr
        )
