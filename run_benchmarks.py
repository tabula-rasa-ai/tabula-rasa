...
def _make_model(d_model=128, n_layers=4, n_heads=4, d_ff=512,
                vocab_size=44, activation='relu',
...
def bench_throughput(model, batch_size=128, seq_len=16, steps=200,
                     warmup=20) -> dict:
...
def bench_architecture(configs: list[dict], steps=200) -> list[dict]:
    ...
    for cfg_dict in configs:
        label = (f"d={cfg_dict.get('d_model',128)} "
...
FULL_CONFIGS = QUICK_CONFIGS + [
    ...
    dict(d_model=128, n_layers=4, n_heads=4, d_ff=512, activation='relu', pos_encoding='rope', norm_type='layernorm'),
]
...
def main():
    ...
    parser.add_argument('--full', action='store_true',
                        help='Full benchmark suite (all configs)')
    parser.add_argument('--throughput', action='store_true',
                        help='Forward-pass throughput only')
    parser.add_argument('--eval', action='store_true',
                        help='Evaluation benchmarks on trained checkpoints')
    parser.add_argument('--op', type=str, default=None,
                        help='Operation to benchmark (add/sub/mul/div)')
    ...
    if args.eval or (not args.throughput and not args.quick):
        ...
        for op, data in eval_results.items():
            if 'error' in data:
                ...
                print(f"  {op}: {data['accuracy_pct']}% acc "
