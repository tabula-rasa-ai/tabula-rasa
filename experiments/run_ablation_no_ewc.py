...
def main():
    ...
    print(
        f"  Retention preserved (<5pp): {'YES ✅' if results['retention_preserved'] else 'NO ❌'}"
    )
    ...
    if ewc_path.exists():
        try:
            ...
            print(f"\n  === VS EWC RUN ===")
            print(
                f"  With EWC:    add={ewc_results.get('addition_with_ewc', ewc_results.get('addition_final_1d', '?'))}%  sub={ewc_results.get('subtraction_acquired', ewc_results.get('subtraction_final_1d', '?'))}%"
            )
            print(f"  Without EWC: add={add_final:.1f}%  sub={sub_final:.1f}%")
            results["ewc_comparison_add"] = ewc_results.get(
                "addition_with_ewc", ewc_results.get("addition_final_1d", None)
            )
            results["ewc_comparison_sub"] = ewc_results.get(
                "subtraction_acquired", ewc_results.get("subtraction_final_1d", None)
            )
